...
if not imguicheck or not sampevcheck or not encodingcheck or not lfscheck or not monetluacheck or not doesFileExist('moonloader/Med Helper/Images/MedH_Images.png') then
	function main()
		...
		sampAddChatMessage(('[MedHelper]{EBEBEB} Началось скачивание необходимых файлов. Если скачивание не удастся, то обратитесь к {ff6633}vk.com/justmini{ebebeb}.'),0xff6633)
...
local Licenses_Arr					= {u8'Лечение',u8'Мото',u8'Рыболовство',u8'Плавание',u8'Оружие',u8'Охоту',u8'Раскопки',u8'Такси'}
							...
							if imgui.IsItemHovered() then
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										...
										MedHelperMessage('Не торопитесь, Вы уже отыгрываете что-то! Прервать отыгровку: {MC}Alt{WC} + {MC}U{WC}')
									end
								end
							end
						end
...
local imgui_settings = imgui.OnFrame(
				...
				elseif mainwindow[0] == 3 then
					...
					elseif infowindow[0] == 2 then
						...
						imgui.BeginGroup()
							...
							imgui.PushFont(font[15])
							imgui.TextColoredRGB('Автор - {MC}#Riki')
	...
	* Меню быстрого доступа - Прицелившись на игрока с помощью ПКМ и нажав кнопку E (по умолчанию), откроется меню быстрого доступа. В данном меню есть все нужные функции, а именно: приветствие, озвучивание прайс листа, продажа лицензий, возможность выгнать человека из больницы, приглашение в организацию, увольнение из организации, изменение должности, занесение в ЧС, удаление из ЧС, выдача выговоров, удаление выговоров, выдача организационного мута, удаление организационного мута, автоматизированное проведение собеседования со всеми нужными отыгровками.
...
local imgui_stats = imgui.OnFrame(
	...
	function(player)
		...
		imgui.Text(fa.ICON_FA_CAR..u8' Пациентов вылечено - '..configuration.my_stats.avto)
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
	...
	elseif dialogId == 235 and getmyrank then
		if find(text, 'Больница') then
			...
			MedHelperMessage('Вы не работаете в больнице, скрипт выгружен! Если это ошибка, то обратитесь к {MC}vk.com/justmini{WC}.')
			NoErrors = true
			thisScript():unload()
		end
...
function checkRules()
	ruless = {}
	for line in lfs.dir(getWorkingDirectory()..'\\Med Helper\\Rules') do
		if line == nil then
		elseif line:match('.+%.txt') then
			local temp = io.open(getWorkingDirectory()..'\\Med Helper\\Rules\\'..line:match('.+%.txt'), 'r+')
			local temptable = {}
			...
			if doesFileExist(json) then
				...
				if f then
					if info[checkServer(select(1, sampGetCurrentServerAddress()))] then
						...
						MedHelperMessage('Ошибка в импортировании правил сервера! Неизвестный сервер.')
					end
				end
			end
		end
			...
			if doesFileExist(json) then
				...
				if f then
					if info[checkServer(select(1, sampGetCurrentServerAddress()))] then
						...
						MedHelperMessage('Ошибка в импортировании устава сервера! Неизвестный сервер.')
					end
				end
			end
		end
...
function main()
	...
	while true do
		...
		if skiporcancel and isKeyDown(0x12) and isKeyJustPressed(0x4F) then
			...
			MedHelperMessage('Ожидание мед. карты остановлено!')
		end
...
changelog = {
	versions = {
...

		{
			...
			date = '01.05.2021',
			text = {
				'Меню статистики /mhmstats сделано более удобным',
				'Двойной клик сохранит местоположение статистики /mhmstats',
				'Изменены кнопки в главном меню /mhm',
				'Теперь можно тестировать цвета в /mhm',
				'Теперь при активации бинда последнее сообщение будет без задержки',
				'Добавлен список изменений',
				'Исправлена проверка обновлений через /mhm',
				'Кардинальный редизайн главного меню /mhm',
 ...
 - Исправлен баг с ударом при принятии человека в организацию]]
...

		{
			...
			text = {
				...
				'Добавлена функция озвучивания лекций /mhmlect',
 			...
 			patches = {
				active = false,
				text = [[
 - Исправлен краш при открытии /mhmstats
 ...
 - Исправлен баг с пропадающим курсором после использования быстрого меню]]
...

		{
			...
			date = '16.05.2021',
			text = {
				'Добавлена функция общения в рацию департамента /mhmdep',
 ...
 - Испралена неработоспособность скрипта из-за перехода сервера на R3]]
		...
		
		{
			...
			date = '11.12.2021',
			text = {
				'Изменение интерфейса в списке изменений и /mhm',
				...
				'Добавлена вкладка \'Отыгровки\', в которой можно настроить: проверку мед. карты при продаже лицензии на оружие, охоту; замену слова \'Больница\' на \'МЦ\'; выставить задержку между сообщениями',
 ...
 - Исправлен баг с неработающей командой /mhmupd
