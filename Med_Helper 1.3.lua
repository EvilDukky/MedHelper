...
if not imguicheck or not sampevcheck or not encodingcheck or not lfscheck or not monetluacheck or not doesFileExist('moonloader/Med Helper/Images/MedH_Images.png') then
	function main()
		...
		sampAddChatMessage(('[MedHelper]{EBEBEB} Началось скачивание необходимых файлов. Если скачивание не удастся, то обратитесь к {FF33F2}t.me/evil_duckky{ebebeb}.'),0xFF33F2)
		...
		if not sampevcheck then -- C официального источника
...
local pricelist = {
	...
	obsl							= new.char[8](tostring(configuration.main_settings.obsl)),
				...
				imgui.BeginChild('##fmmainwindow', imgui.ImVec2(300, -1), false)
						...
						if clienttype[0] == 0 then
							...
							imgui.BeginGroup()
								...
								if configuration.main_settings.myrankint >= 2  then
									if imgui.Button(fa.ICON_FA_FILE_ALT..u8' Быстрое лечение', imgui.ImVec2(285,30)) then
										sendchatarray(configuration.main_settings.playcd, {
									...
									imgui.LockedButton(fa.ICON_FA_FILE_ALT..u8' Быстрое лечение', imgui.ImVec2(285,30))
									imgui.Hint('firstrangpricelist', 'Со 2-го ранга')
								end
								if configuration.main_settings.myrankint >= 1  then
									if imgui.Button(fa.ICON_FA_FILE_ALT..u8' Обследование', imgui.ImVec2(285,30)) then
										sendchatarray(configuration.main_settings.playcd, {
											{'Прекрасно, что вы решили пройти медицинскую проверку.'},
											{'У вас могут быть некоторые проблемы со здоровьем.'},
											{'Некоторые недуги могут серьёзно ограничить ваши способности к службе!'},
											{'/do Медицинская сумка на плече %s.', #configuration.main_settings.myname < 1 and gsub(sampGetPlayerNickname(select(2,sampGetPlayerIdByCharHandle(playerPed))), '_', ' ') or u8:decode(configuration.main_settings.myname)},
											{'/me правой рукой {gender:расстегнул|расстегнула} медицинскую сумку и достал нужные средства'},
											{'/todo Пожалуйста, расслабьтесь, это займет лишь некоторое время*приступая к обследованию'},
											{'Я буду внимательно следить за вашими показателями.'},
											{'Если потребуется, я дам рекомендации по дальнейшему обследованию.'},
											{'Ваша безопасность и здоровье - мой главный приоритет!'},
											{'/mticket %s %s',fastmenuID, configuration.main_settings.obsl},
										})
									end
								else
									imgui.LockedButton(fa.ICON_FA_FILE_ALT..u8' Обследование', imgui.ImVec2(285,30))
									imgui.Hint('firstrangpricelist', 'Со 2-го ранга')
								end
								if configuration.main_settings.myrankint >= 1  then
									if imgui.Button(fa.ICON_FA_FILE_ALT..u8' Выдать антибиотики', imgui.ImVec2(285,30)) then
										sendchatarray(configuration.main_settings.playcd, {
											...
											{'/me {gender:положил|положила} лист бумаги и ручку в шкафчик,после чего {gender:закрыл|закрыла} его'},
						...
						if medtap[0] == 1 then
							...
							if (med_results.pass == 'меньше 4 лет в штате') then
								...
								if imgui.Button(u8'Продолжить '..fa.ICON_FA_ARROW_RIGHT, imgui.ImVec2(285,30)) then
									if not inprocess then
										sendchatarray(configuration.main_settings.playcd, {
											{'/todo Благодарю вас!*взяв паспорт в руки и {gender:начал|начала} его изучать'},
							...
							if imgui.IsItemHovered() then
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										local result, mid = sampGetPlayerIdByCharHandle(playerPed)
										sendchatarray(configuration.main_settings.playcd, {
											{'Стоимость мед.осмотра - %s$.', string.separate(configuration.main_settings.osm)},
							...
							if imgui.IsItemHovered() then
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										local result, mid = sampGetPlayerIdByCharHandle(playerPed)
										sendchatarray(configuration.main_settings.playcd, {
											{'Перед сеансом лечения я {gender:должен|должна} вас уведомить о ценовой политике данной процедуры.'},
								...
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										sendchatarray(configuration.main_settings.playcd, {
											...
											{'/me {gender:взял|взяла} штамп в руки и {gender:поставил|поставила} штамп %s', configuration.main_settings.replaceash and '{location:ЛСМЦ|СФМЦ|ЛВМЦ|ДФМЦ}' or '{location:Больницы Лос-Сантос|Больницы Сан-Фиерро|Больницы Лас-Вентурас|Больницы Джефферсон}'},
					...
					elseif newwindowtype[0] == 7 then
						...
						if tatutap[0] == 1 then
							...
							imgui.Separator()
							imgui.SetCursorPosX(7.5)
							imgui.Button(u8'Покажите тату..', imgui.ImVec2(285,30))
							if imgui.IsItemHovered() then
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										sendchatarray(configuration.main_settings.playcd, {
								...
								if imgui.IsMouseReleased(0) then
									if not inprocess then
										sendchatarray(configuration.main_settings.playcd, {
											...
											{'/me взяв аппарат, {gender:осмотрел|осмотрела} пациента и {gender:принялся|принялась} выводить татуировку'},
					...
					elseif newwindowtype[0] == 10 then
						...
						if sobesetap[0] == 0 then
							...
							if imgui.Button(u8'Поприветствовать', imgui.ImVec2(285,30)) then
								sendchatarray(configuration.main_settings.playcd, {
									{'Здравствуйте, я %s %s, Вы пришли на собеседование?', configuration.RankNames[configuration.main_settings.myrankint], configuration.main_settings.replaceash and '{location:ЛСМЦ|СФМЦ|ЛВМЦ|ДФМЦ}' or '{location:Больницы Лос-Сантос|Больницы Сан-Фиерро|Больницы Лас-Вентурас|Больницы Джефферсон}'},
									{'/do На груди висит бейджик с надписью %s %s.', configuration.RankNames[configuration.main_settings.myrankint], #configuration.main_settings.myname < 1 and gsub(sampGetPlayerNickname(select(2,sampGetPlayerIdByCharHandle(playerPed))), '_', ' ') or u8:decode(configuration.main_settings.myname)},
								})
							end
							imgui.SetCursorPosX(7.5)
							...
							imgui.BeginGroup()
								...
								if imgui.Button(fa.ICON_FA_USER..u8' Убрать из чёрного списка', imgui.ImVec2(275,30)) then
									...
									sendchatarray(configuration.main_settings.playcd, {
										...
										{'/me {gender:подтвердил|подтвердила} изменения'},
								...
								if imgui.Button(fa.ICON_FA_VOLUME_UP..u8' Снять мут сотруднику', imgui.ImVec2(275,30)) then
									...
									sendchatarray(configuration.main_settings.playcd, {
										{'/me {gender:достал|достала} планшет из кармана'},
										{'/me {gender:включил|включила} планшет'},
										{'/me {gender:перешёл|перешла} в раздел \'Управление сотрудниками %s\'', configuration.main_settings.replaceash and '{location:ЛСМЦ|СФМЦ|ЛВМЦ|ДФМЦ}' or '{location:Больницы Лос-Сантос|Больницы Сан-Фиерро|Больницы Лас-Вентурас|Больницы Джефферсон}'},
									...
									if #str(uninvitebuf) > 0 then
										if uninvitebox[0] then
											...
											sendchatarray(configuration.main_settings.playcd, {
												...
												{'/do Раздел открыт.'},
												{'/me {gender:внёс|внесла} человека в раздел \'Увольнение\''},
												{'/me {gender:подтвердил|подтвердила} изменения, затем {gender:выключил|выключила} планшет и {gender:положил|положила} его обратно в карман'},
												{'/uninvite %s %s', fastmenuID, u8:decode(str(uninvitebuf))},
											})
										end
									else
			...
			imgui.Image(medh_image,imgui.ImVec2(198,25),imgui.ImVec2(0.25,configuration.main_settings.style ~= 2 and 0.8 or 0.9),imgui.ImVec2(1,configuration.main_settings.style ~= 2 and 0.9 or 1))
				...
				if imgui.BeginPopupModal(u8'Все команды', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
					...
					imgui.BeginGroup()
						...
						imgui.TextColoredRGB('/heal [id] [цена] - Вылечить человека (1+)')
						imgui.TextColoredRGB('/cure [id] - Реанимировать человека (1+)')
					imgui.EndGroup()
					imgui.Spacing()
					...
					imgui.BeginChild('##usersettingsmainwindow',_,false)
							...
							imgui.BeginGroup()
								...
								imgui.BeginGroup()
									...
									imgui.BeginGroup()
										...
										imgui.PushStyleVarVec2(imgui.StyleVar.WindowPadding,imgui.ImVec2(10,10))
										if imgui.Combo(u8'##chooselocationcombo',usersettings.location, new['const char*'][4]({u8'Больница ЛС',u8'Больница СФ',u8'Больница ЛВ',u8'Больница ДФ'}), 4) then
						...
						elseif settingswindow[0] == 3 then
							...
							imgui.BeginGroup()
								...
								if imgui.InputText(u8'Обследование', pricelist.obsl, sizeof(pricelist.obsl), imgui.InputTextFlags.CharsDecimal) then
									configuration.main_settings.obsl = str(pricelist.obsl)
									inicfg.save(configuration,'Med Helper')
								end
				...
				elseif mainwindow[0] == 3 then
					...
					elseif infowindow[0] == 2 then
						...
						imgui.BeginGroup()
							...
							imgui.SameLine(190)
							imgui.Link('https://t.me/evil_duckky', u8't.me/evil_duckky')
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
	...
	elseif dialogId == 235 and getmyrank then
		if find(text, 'Больница') then
			...
			MedHelperMessage('Вы не работаете в больнице, скрипт выгружен! Если это ошибка, то обратитесь к {MC}t.me/evil_duckky{WC}.')
			NoErrors = true
			thisScript():unload()
		end
...
5. Если даже это не помогло Вам, то отправьте автору {2594CC}(t.me/evil_duckky){FF33F2} скриншот ошибки.{FFFFFF}
———————————————————————————————————————————————————————
...
function main()
	...
	sampRegisterChatCommand('heal', function(param)
		if sampIsPlayerPaused(id) then
			return MedHelperMessage('Игрок находится в АФК!')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			...
			{'/me аккуратным движением руки {gender:передал|передала} лекарство пациенту'},
			{'Принимайте эти таблетки, и через некоторое время вам станет лучше.'},
			{'/heal %s %s',param,configuration.main_settings.heal},
		})
	end)

	sampRegisterChatCommand('cure', function(param)
		if sampIsPlayerPaused(id) then
			return MedHelperMessage('Игрок находится в АФК!')
		end
		if configuration.main_settings.myrankint < 1 then
			return MedHelperMessage('Данная команда доступна с 1-го ранга.')
		end
		return sendchatarray(configuration.main_settings.playcd, {
			{'/todo Что-то ему вообще не хорошо*снимая медицинскую сумку с плеча'},
			{'/me ставит медицинскую сумку возле пострадавшего'},
...
changelog = {
 ...
 - Убрана система вакцинации от коронавируса]]
		...
		
		{
			...
			date = '23.11.2024',
			text = {
				'Исправления незначительных орфографических ошибок',
				'Изменены доступы к командам',
				'Добавлена система медицинского обследования',
				'Изменены контакты для связи',
			},
			patches = {
				active = false,
				text = [[
 - Убраны остатки системы вакцинации от коронавируса]]
			},
		},
